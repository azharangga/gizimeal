...
export const Route = createFileRoute("/predict/result/menu/$slug")({
  component: MenuPage,
  head: () => ({
    meta: [
      { title: "Resep & Cara Memasak - GiziMeal" },
      {
        name: "description",
        content:
          "Resep, bahan, dan informasi gizi untuk menu rekomendasi GiziMeal.",
      },
    ],
  }),
});
...
function MenuPage() {
  const { slug } = useParams({ from: "/predict/result/menu/$slug" });
  ...
  useEffect(() => {
    ...
    setHydrated(true);
    if (!m) {
      navigate({ to: "/predict/result", replace: true });
    }
  }, [slug, navigate]);
